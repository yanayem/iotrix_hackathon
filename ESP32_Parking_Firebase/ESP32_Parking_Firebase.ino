...
void setup() {
  ...
  xTaskCreatePinnedToCore(
    ...
    1,
    &NetworkTaskHandle,
    0  // Core 0: WiFi, Firebase, HTTP Server
  );
...
// =====================================================
// SENSOR TASK (Core 1)
// RFID, IR Sensors, Flame, Gas, OLED, Servo
// =====================================================
...
void sensorTask(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (currentMillis - lastSensorCheck >= sensorInterval) {
      ...
      // PARKING
      slotAOccupied = (digitalRead(slotA_sensor) == LOW);
      slotBOccupied = (digitalRead(slotB_sensor) == LOW);

      // GAS
      gasValue = analogRead(gasSensorPin);
      gasDanger = (gasValue >= GAS_DANGER_LEVEL);

      // FIRE
      fireDetected = (digitalRead(fireSensorPin) == FIRE_ACTIVE_STATE);

      xSemaphoreGive(dataMutex);

      // PARKING STATE CHANGE
      if (slotAOccupied != previousSlotA || slotBOccupied != previousSlotB) {
        Serial.println();
        Serial.println("PARKING STATUS CHANGED");
        printParkingStatus();

        previousSlotA = slotAOccupied;
        previousSlotB = slotBOccupied;

        if (!showingAccessMessage && !showingGateClosedMessage && !showingSafetyAlert) {
          updateNormalOLED();
        }
      }

      // SAFETY STATE CHANGE
      if (gasDanger != previousGasDanger || fireDetected != previousFireDetected) {
        Serial.println();
        Serial.println("SAFETY STATUS CHANGED");
        printSafetyStatus();

        previousGasDanger = gasDanger;
    ...
    if (gateOpen) {
      if (currentMillis - gateOpenedAt >= gateOpenTime) {
        closeGate();
      }
    }
    ...
    if (showingAccessMessage) {
      if (currentMillis - accessMessageStarted >= accessMessageTime) {
        showingAccessMessage = false;
        if (!gateOpen && !showingGateClosedMessage && !showingSafetyAlert) {
          lastOLEDState = -1;
          updateNormalOLED();
        }
      }
    ...
    if (showingGateClosedMessage) {
      if (currentMillis - gateClosedMessageStarted >= gateClosedMessageTime) {
        showingGateClosedMessage = false;
    ...
    if (!gateOpen && !showingAccessMessage && !showingGateClosedMessage) {
...
// =====================================================
// LOCAL DASHBOARD HTML (served by ESP32 HTTP Server)
// =====================================================
...
String getLocalDashboardHTML() {
  ...
  html += "<style>";
  html += "body{font-family:sans-serif;background:#f0fdf4;text-align:center;padding:20px;margin:0;}";
  html += "h1{color:#0d9488;margin-bottom:20px;}";
  html += ".grid{display:flex;flex-wrap:wrap;justify-content:center;gap:15px;}";
  html += ".card{background:white;padding:20px;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1);min-width:200px;border-top:4px solid #14b8a6;}";
  html += ".ok{color:#16a34a;font-weight:bold;} .danger{color:#dc2626;font-weight:bold;}";
  html += ".ip{background:#0d9488;color:white;padding:8px 16px;border-radius:8px;display:inline-block;margin-top:15px;}";
  html += "</style></head><body>";
  html += "<h1>ESP32 Live Dashboard</h1>";
  html += "<div class='grid'>";

  html += "<div class='card'><h3>Slot A</h3><p class='" + String(sa ? "danger" : "ok") + "'>" + String(sa ? "OCCUPIED" : "EMPTY") + "</p></div>";
  html += "<div class='card'><h3>Slot B</h3><p class='" + String(sb ? "danger" : "ok") + "'>" + String(sb ? "OCCUPIED" : "EMPTY") + "</p></div>";
  html += "<div class='card'><h3>Free Slots</h3><p class='ok'>" + String(free) + " / 2</p></div>";
  html += "<div class='card'><h3>Gate</h3><p class='" + String(go ? "ok" : "danger") + "'>" + String(go ? "OPEN" : "CLOSED") + "</p></div>";
  html += "<div class='card'><h3>Gas Sensor</h3><p class='" + String(gd ? "danger" : "ok") + "'>" + String(gd ? "DANGER" : "NORMAL") + "</p><p>Value: " + String(gv) + "</p></div>";
  html += "<div class='card'><h3>Fire Sensor</h3><p class='" + String(fd ? "danger" : "ok") + "'>" + String(fd ? "FIRE DETECTED" : "NORMAL") + "</p></div>";

  html += "</div>";
  html += "<div class='ip'>IP: " + WiFi.localIP().toString() + "</div>";
  html += "</body></html>";
  return html;
}
